  ...
  return (
    <div className="flex flex-col gap-8">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-3xl font-black tracking-tight text-foreground">KELOLA TERMINAL SHELL</h2>
          ...
          <div className="border border-border-custom bg-card-custom p-6 rounded-2xl flex flex-col gap-4">
            ...
            <AnimatePresence>
              {terminalFormOpen && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: -10 }}
                  className="border border-foreground/15 bg-background p-4 rounded-xl flex flex-col gap-3 shadow-sm mb-4"
                >
                  ...
                  <form onSubmit={handleSaveTerminal} className="flex flex-col gap-3">
                    <div className="flex flex-col gap-1">
                      ...
                      <input
                        ...
                        className="bg-background border border-border-custom rounded-lg px-3 py-2 text-xs focus:outline-none text-foreground font-mono"
                      />
                    </div>
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <textarea
                        ...
                        className="bg-background border border-border-custom rounded-lg px-3 py-2 text-xs focus:outline-none text-foreground font-mono resize-none"
                      />
                    </div>
                    ...
                    <div className="flex justify-end gap-2 mt-1">
                      <button
                        type="button"
                        onClick={() => setTerminalFormOpen(false)}
                        className="border border-border-custom px-3 py-1.5 rounded-lg text-[10px] font-bold hover:bg-accent-light"
                      >
                        Batal
                      </button>
                      <button
                        type="submit"
                        className="bg-foreground text-background px-4 py-1.5 rounded-lg text-[10px] font-bold hover:bg-accent-hover"
                      >
                        Simpan Perintah
                      </button>
                    </div>
            ...
            <div className="flex flex-col gap-3">
              {terminalCommands.length === 0 ? (
                <p className="text-xs text-accent-custom text-center py-6">
                  {"Belum ada perintah kustom. Pemicu 'help', 'clear', dan 'theme' adalah perintah bawaan."}
                </p>
              ) : (
                terminalCommands.map((cmd) => (
                  <div
                    key={cmd.id}
                    className="border border-border-custom bg-background p-4 rounded-xl flex items-center justify-between gap-4 hover:border-foreground/15 transition-all"
                  >
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <button
                        ...
                        className="h-7 w-7 rounded-full border border-border-custom flex items-center justify-center hover:bg-accent-light text-foreground"
                        title="Edit Perintah"
                      >
                        <Edit2 className="h-3 w-3" />
                      </button>
                      <button
                        onClick={() => deleteTerminalCommand(cmd.id)}
                        className="h-7 w-7 rounded-full border border-border-custom flex items-center justify-center hover:bg-red-500/10 hover:border-red-500/30 hover:text-red-500"
            ...
            <form onSubmit={handleSaveTerminalConfig} className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1">
                ...
                <textarea
                  ...
                  className="bg-background border border-border-custom rounded-lg px-3 py-2 text-xs focus:outline-none text-foreground font-mono resize-none leading-relaxed"
                />
              </div>
