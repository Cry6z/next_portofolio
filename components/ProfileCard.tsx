...
const DEFAULT_INNER_GRADIENT = 'linear-gradient(145deg,#111111 0%,#333333 100%)';
...
const ProfileCardComponent: React.FC<ProfileCardProps> = ({
  ...
  const cardStyle = useMemo(
    () => ({
      ...
      // Minimalist Monochromatic Glow Colors
      '--sunpillar-1': 'hsl(0, 0%, 20%)',
      '--sunpillar-2': 'hsl(0, 0%, 40%)',
      '--sunpillar-3': 'hsl(0, 0%, 60%)',
      '--sunpillar-4': 'hsl(0, 0%, 80%)',
      '--sunpillar-5': 'hsl(0, 0%, 100%)',
      '--sunpillar-6': 'hsl(0, 0%, 50%)',
  ...
  const shineStyle = {
    ...
    filter: 'brightness(0.66) contrast(1.33) saturate(0) opacity(0.5)',
  ...
  return (
    <div
      ...
      style={{ perspective: '800px', transform: 'translate3d(0, 0, 0.1px)', ...cardStyle } as React.CSSProperties}
    >
      {behindGlowEnabled && (
        <div
      ...
      <div ref={shellRef} className="relative z-1 group">
        ...
        >
          ...
          >
            {/* Removed shine and glare layers per user request */}
            ...
            {/* Details content */}
            <div
              className="max-h-full overflow-hidden text-center relative z-5 hidden"
              style={{
                transform:
            ...
            >
              {/* Hidden detailed title/name from ReactBits since we already have it in Hero section */}
            </div>
          </div>
        </section>
      </div>
